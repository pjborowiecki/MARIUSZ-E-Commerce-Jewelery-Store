        ...
        rejectedFiles.forEach(({ errors }) => {
          if (errors[0]?.code === "file-too-large") {
            toast({
              ...
              description: `Maksymalny rozmiar to ${formatBytes(maxSize)}`,
              variant: "destructive",
            })
            return
          }
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <Button variant="outline" className="bg-secondary" disabled={disabled}>
          Dodaj zdjęcia
          ...
          {isUploading ? (
            ...
            <div className="grid place-items-center gap-2 text-muted-foreground sm:px-5">
              <Icons.upload
                ...
                aria-hidden="true"
              />
              <p className="text-base font-medium">Wrzuć plik w tym miejscu</p>
            </div>
          ) : (
            <div className="grid place-items-center gap-1 sm:px-5">
              ...
              <p className="text-sm text-slate-500">
                Rozmiar pliku nie może przekraczać {formatBytes(maxSize)}
              </p>
            </div>
          )}
        </div>
        <p className="text-center text-sm font-medium text-muted-foreground">
          Maksymalna ilość plików: {maxFiles}
        </p>
        {files?.length ? (
          ...
          <Dialog open={isOpen} onOpenChange={setIsOpen}>
            <DialogTrigger asChild>
              ...
              >
                ...
                <span className="sr-only">Przytnij obraz</span>
              </Button>
            </DialogTrigger>
            <DialogContent>
              <p className="absolute left-5 top-4 text-base font-medium text-muted-foreground">
              ...
              <div className="mt-8 grid place-items-center space-y-5">
                ...
                <div className="flex items-center justify-center space-x-2">
                  ...
                  <Button
                    aria-label="Zresetuj przycinanie"
                    type="button"
                    variant="outline"
                    size="sm"
                    className="h-8"
